...
class Component implements Component_Interface, Templating_Component_Interface {
	...
	public function action_enqueue_scripts() {
		...
		foreach ( $js_files as $handle => $data ) {
			...
			$version = TTCMA()->get_asset_version( $js_dir . $data['file'] );
		...
		$handles  = array_filter(
			...
			function ( $handle ) use ( $js_files ) {
				...
				if ( ! $is_valid ) {
					...
					_doing_it_wrong( __CLASS__ . '::print_scripts()', esc_html( sprintf( __( 'Invalid theme JS handle: %s', 'TTCMA' ), $handle ) ), 'TTCMA 2.0.0' );
				}
				return $is_valid;
			}
		);
	...
	protected function get_js_files() : array {
		 ...
		 */
		$js_files = apply_filters( 'TTCMA_js_files', $js_files );
