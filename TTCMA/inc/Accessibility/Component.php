...
class Component implements Component_Interface {
	...
	public function action_enqueue_navigation_script() {
		...
		wp_enqueue_script(
			...
			array(),
			TTCMA()->get_asset_version( get_theme_file_path( '/assets/js/navigation.min.js' ) ),
			false
		);
		wp_script_add_data( 'TTCMA-navigation', 'async', true );
		wp_script_add_data( 'TTCMA-navigation', 'precache', true );
		wp_localize_script(
			'TTCMA-navigation',
			'tTCMAScreenReaderText',
			array(
				'expand'   => __( 'Expand child menu', 'TTCMA' ),
				'collapse' => __( 'Collapse child menu', 'TTCMA' ),
			)
		);
	}
