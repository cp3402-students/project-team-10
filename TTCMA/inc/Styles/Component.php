...
use function WP_Rig\WP_Rig\TTCMA;
...
class Component implements Component_Interface, Templating_Component_Interface {
	...
	/**
	 * Gets template tags to expose as methods on the Template_Tags class instance, accessible through `TTCMA()`.
	 *
	...
	public function action_enqueue_styles() {
		...
		if ( ! empty( $google_fonts_url ) ) {
			wp_enqueue_style( 'TTCMA-fonts', $google_fonts_url, array(), null ); // phpcs:ignore WordPress.WP.EnqueuedResourceParameters.MissingVersion
		}
		...
		foreach ( $css_files as $handle => $data ) {
			...
			$version = TTCMA()->get_asset_version( $css_dir . $data['file'] );
	...
	public function filter_resource_hints( array $urls, string $relation_type ) : array {
		if ( 'preconnect' === $relation_type && wp_style_is( 'TTCMA-fonts', 'queue' ) ) {
			$urls[] = array(
		...
		$handles   = array_filter(
			...
			function( $handle ) use ( $css_files ) {
				...
				if ( ! $is_valid ) {
					...
					_doing_it_wrong( __CLASS__ . '::print_styles()', esc_html( sprintf( __( 'Invalid theme stylesheet handle: %s', 'TTCMA' ), $handle ) ), 'TTCMA 2.0.0' );
				}
				return $is_valid;
			}
		);
	 ...
	 * {@see 'TTCMA_preloading_styles_enabled'} filter can be used to tweak the return value.
	 *
	...
	protected function preloading_styles_enabled() {
		$preloading_styles_enabled = ! TTCMA()->is_amp();
		 ...
		 */
		return apply_filters( 'TTCMA_preloading_styles_enabled', $preloading_styles_enabled );
	}
	...
	protected function get_css_files() : array {
		...
		$css_files = array(
			...
			'TTCMA-sidebar'    => array(
				...
				'preload_callback' => function() {
					return TTCMA()->is_primary_sidebar_active();
				},
			),
			'TTCMA-widgets'    => array(
		 ...
		 */
		$css_files = apply_filters( 'TTCMA_css_files', $css_files );
	...
	protected function get_google_fonts() : array {
		 ...
		 */
		$this->google_fonts = (array) apply_filters( 'TTCMA_google_fonts', $google_fonts );
