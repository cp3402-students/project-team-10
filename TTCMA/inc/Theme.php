...
class Theme {
			...
			if ( ! $component instanceof Component_Interface ) {
				throw new InvalidArgumentException(
					sprintf(
						...
						__( 'The theme component %1$s does not implement the %2$s interface.', 'TTCMA' ),
						gettype( $component ),
	 ...
	 *
	 * Calling `TTCMA()` is a short-hand for calling this method on the main theme instance. The instance then allows
	...
	public function component( string $slug ) : Component_Interface {
		if ( ! isset( $this->components[ $slug ] ) ) {
			throw new InvalidArgumentException(
				sprintf(
					/* translators: %s: slug */
					__( 'No theme component with the slug %s exists.', 'TTCMA' ),
					$slug
				)
			);
		}
